...
- **Recall**: proportion of true positives detected out of all actual positives (true positive rate or sensitivity; *TPs / (TPs + FNs)*).
- **Precision**: proportion of true positives among all predicted positives (*TPs / (TPs + FPs)*).
- **F1 score**: harmonic mean of precision and recall, providing a single measure that balances both (also known as the F-measure).
...
All [ohun](https://github.com/ropensci/ohun) functions that work with this kind of data can take both selection tables and data frames. Spectrograms with highlighted sound events from a selection table can be plotted with the function `label_spectro()` (this function only plots one wave object at the time):
